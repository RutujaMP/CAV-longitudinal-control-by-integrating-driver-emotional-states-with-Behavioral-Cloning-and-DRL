...
# class Main(Config):
#     flow_base = os.path.abspath(os.getenv('F', ''))

#     never_save = {'trial', 'has_av', 'e', 'disable_amp', 'opt_level', 'tmp'} | Config.never_save

#     def __init__(self, res, *args, **kwargs):
#         tmp = os.path.abspath(res) in [os.path.expanduser('~'), Path(Main.flow_base), Path(Main.flow_base) / 'all_results']
#         if tmp:
#             res = Path(Main.flow_base) / 'tmp' / rand_string(8)
...
#             res.mk()  # Assuming res is a Path object or has a method mk()
...
#         if c.wb:
#             import wandb
#             wandb_id_path = (c.res / 'wandb' / 'id.txt').mkdir(parents=True, exist_ok=True)
#             c._wandb_run = wandb.init(  # name and project should be set as env vars
#                 name=c.res.relative_to(Path(os.environ.get('FA'))),
#                 dir=c.res,
#                 id=wandb_id_path.read_text() if wandb_id_path.exists() else None,
#                 config={k: v for k, v in c.items() if not k.startswith('_')},
#                 save_code=False
#             )
#             wandb_id_path.write_text(c._wandb_run.id)
#             c._writer = wandb
...
#     def eval(c):
#         vehicle_info_path = Path(c.vehicle_info_save)
#         net_xml_path = (vehicle_info_path.stem + '.net.xml')
#         #net_xml_path = vehicle_info_path.parent / (vehicle_info_path.stem + '.net.xml')


#         # Ensure c._env.sumo_paths['net'] is treated as a Path object
#         if isinstance(c._env.sumo_paths['net'], Path):
#             shutil.copy(c._env.sumo_paths['net'], str(net_xml_path))
#         else:
#             raise TypeError("sumo_paths['net'] should be a Path object.")

#         # Set defaults for c
#         c.setdefaults(alg='PPO')

#         # Recreate environment
#         c._env = c.create_env(c)

#         # Initialize algorithm based on c.alg
#         c._alg = (eval(c.alg) if isinstance(c.alg, str) else c.alg)(c)

#         # Set model and put it in evaluation mode
#         c.set_model()
#         c._model.eval()

#         # Initialize results DataFrame and writer buffer
#         c._results = pd.DataFrame(index=pd.Series(name='step'))
#         c._writer_buffer = NamedArrays()

#         # Set state based on evaluation parameters
...
#         # Start running the evaluation loop
#         c._run_start_time = time()
#         c._i = 1
...
#                 shutil.copy(c._env.sumo_paths['net'], c.vehicle_info_save.replace('.npz', '.net.xml'))  # Ensure net.xml is copied
#             c._i += 1
#             c.log('')
...
#             if c.get('use_ray', True) and c.n_rollouts_per_step > 1 and c.get('n_workers', float('inf')) > 1:
...
#                 except:
#                     ray.init(num_cpus=c.n_workers, include_dashboard=False, _temp_dir=(Path(os.environ.get('F')) / 'tmp').resolve())
...
from automatic_vehicular_control.u import Config
from automatic_vehicular_control.env_factory import create_ramp_env  # Import the factory function
from automatic_vehicular_control.ut import *
import shutil
import numpy as np
import pandas as pd
...
class Main(Config):
    ...
    def __init__(c, res, mode='train', *args, **kwargs):  # Add mode parameter with default value 'train'
        c.mode = mode  # Set the mode in the config
        home_path = Path.env('HOME') if 'HOME' in os.environ else Path('C:/Users/Default')  # Default to 'C:/Users/Default' if 'HOME' is not set
        tmp = Path(res)._real in [home_path, Main.flow_base, Main.flow_base / 'all_results']
        if tmp:
    ...
    def create_env(c):
        if c.mode == 'train':
            return create_ramp_env(training=True)  # Training environment
        else:
            return create_ramp_env(training=False)  # Evaluation environment

    @property
    def dist_class(c):
        if '_dist_class' not in c.__dict__:
            c._dist_class = build_dist(c._env.action_space)
        return c._dist_class
    ...
    @property
    def observation_space(c):
        return c._env.observation_space  # Ensure this reflects any HDV-specific states

    @property
    def action_space(c):
        return c._env.action_space
    ...
    def on_rollout_worker_start(c):
        c._env = c.create_env(c)
        assert c._env.action_space is not None, "Action space must be defined before setting the model"
        c.use_critic = False  # Don't need value function on workers
        c.set_model()
        c._model.eval()
        c._i = 0
    ...
    def eval(c):
        ...
        for _ in range(c.n_steps):
            ...
            if c.get('vehicle_info_save'):
                ...
                if c.get('save_agent'):
                    np.savez_compressed(c.vehicle_info_save.replace('.npz', '_agent.npz'), **{k: v.values.astype(type(v.iloc[0])) for k, v in c._env.agent_info.items()})
                net_path = c._env.sumo_paths['net']
                target_path = c.vehicle_info_save.replace('.npz', '.net.xml')
                shutil.copyfile(net_path, target_path)
            c._i += 1
            c.log('')
        if hasattr(c._env, 'close'):
            c._env.close()
