...
# Load and plot human speed data
for label in ['Ours (DRL)', 'Ours (Derived)']:
        ...
        if k[0] == label:
            ...
            if referenced_data is not None:
                try:
                    ...
                    # Plot human speed data
                    plt.plot(step_data[human_mask], speed_human, label=f'{label} - Speed Human')
                except Exception as e:
...
plt.ylabel('Speed')
plt.legend(loc='upper right')  # Fixed legend location to speed up plotting
plt.grid()
plt.title('Human Speed vs Iterations')
plt.tight_layout()
plt.show()
